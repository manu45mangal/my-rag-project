...
var PineconeEmbeddings = class extends _langchain_core_embeddings.Embeddings {
	client;
	model;
	params;
	constructor(fields) {
		...
		super(defaultFields);
		if (defaultFields.apiKey) this.client = require_client.getPineconeClient({
	...
	async embedDocuments(texts) {
		...
		if (this.params) embeddings = await this.caller.call(async () => {
			return await this.client.inference.embed(this.model, texts, this.params);
		});
		else embeddings = await this.caller.call(async () => {
			return await this.client.inference.embed(this.model, texts, {});
		});
