...
* <br />
*/
var PineconeStore = class PineconeStore extends _langchain_core_vectorstores.VectorStore {
	textKey;
	namespace;
	pineconeIndex;
	filter;
	...
	constructor(embeddings, params) {
		...
		else if (pineconeConfig) this.pineconeIndex = new _pinecone_database_pinecone.Index(pineconeConfig.indexName, {
		...
		this.filter = filter;
		this.caller = new _langchain_core_utils_async_caller.AsyncCaller(asyncCallerArgs);
	}
	/**
	...
	*/
	async addVectors(vectors, documents, options) {
		...
		const documentIds = ids == null ? documents.map(() => (0, _langchain_core_utils_uuid.v4)()) : ids;
		const pineconeVectors = vectors.map((values, idx) => {
			...
			const metadata = {
				...(0, flat.default)(documentMetadata),
				...stringArrays,
		...
		const batchRequests = (0, _langchain_core_utils_chunk_array.chunkArray)(pineconeVectors, 100).map((chunk) => this.caller.call(async () => {
			try {
	...
	*/
	...
	async _runPineconeQuery(query, k, filter, options) {
		...
		return await this.pineconeIndex.namespace(optionsNamespace ?? "").query({
	...
	*/
	_formatMatches(matches = []) {
		...
		for (const record of matches) {
			...
			if (score) documentsWithScores.push([new _langchain_core_documents.Document({
				id,
	...
	*/
	async similaritySearchVectorWithScore(query, k, filter) {
		...
		return this._formatMatches(matches);
	}
	/**
	...
	*/
	async maxMarginalRelevanceSearch(query, options) {
		...
		const { matches = [] } = await this._runPineconeQuery(queryEmbedding, options.fetchK ?? 20, options.filter, { includeValues: true });
		const topMmrMatches = (0, _langchain_core_utils_math.maximalMarginalRelevance)(queryEmbedding, matches.map((match) => match.values), options.lambda, options.k).map((idx) => matches[idx]);
		return this._formatMatches(topMmrMatches).map(([doc, _score]) => doc);
	}
	/**
